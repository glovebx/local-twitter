...
/**
 * 1. Successful change to liked
 * 2. Successful change to unliked
...
class ToggleLikeTest {
    ...
    // system in test
    private lateinit var toggleLike: ToggleLike
    ...
    @Test
    fun successful_like() = runBlocking {
        ...
        // toggle like
        val emissions = toggleLike.execute(
            id = post.id
        ).toList()
    ...
    @Test
    fun successful_unlike() = runBlocking {
        // condition the response
        mockWebServer.enqueue(
            MockResponse()
                .setResponseCode(HttpURLConnection.HTTP_OK)
